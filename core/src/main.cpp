...
void parseArguments(int argc, char *argv[], app::InputParams &params)
{
    ...
    params.optionType = app::OptionType::call;
    ...
    try
    {
        while ((opt = getopt(argc, argv, "S:K:r:q:T:s:n:cpi")) != -1)
        {
            ...
            {
            case 'S':
                params.underlyingPrice = std::stod(optarg);
                has_S = true;
                break;
            case 'K':
                params.strike = std::stod(optarg);
                has_K = true;
                break;
            case 'r':
                params.riskFreeRate = std::stod(optarg);
                has_r = true;
                break;
            case 'q':
                params.dividendYield = std::stod(optarg);
                break;
            case 'T':
                params.daysToExpiration = std::stod(optarg);
                has_T = true;
                break;
            case 's':
                params.sigma = std::stod(optarg);
                has_s = true;
                break;
            case 'n':
            ...
            case 'c':
                ...
                break;
            case 'p':
                params.optionType = app::OptionType::put;
                break;
            case 'i':
    ...
    if (interactiveMode)
    {
        ...
        int optionType;
        std::cin >> optionType;
        params.optionType = optionType == 1 ? app::OptionType::call
                                            : app::OptionType::put;
    }
}
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        std::cout << std::setprecision(10) << "{\"price\": " << option_price.price
